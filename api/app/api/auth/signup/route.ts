...
export async function POST(req: NextRequest) {
  try {
    ...
    await run(
      `INSERT INTO User (id, email, name, username, password_hash, image, chips, createdAt, updatedAt)
       VALUES (?, ?, ?, ?, ?, NULL, 1000, ?, ?)`,
    ...
    const user = await one<any>("SELECT id, email, name, username, image, chips FROM User WHERE id = ?", [id]);
