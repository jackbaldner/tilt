...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await transaction((db) => {
    db.run("INSERT INTO BetSide (id, betId, userId, option, stake, status, createdAt) VALUES (?, ?, ?, ?, ?, 'active', ?)", [sideId, betId, auth.id, option, bet.stake, timestamp]);
    ...
    db.run(`INSERT INTO "Transaction" (id, userId, betId, type, amount, description, createdAt) VALUES (?, ?, ?, 'bet_placed', ?, ?, ?)`, [cuid(), auth.id, betId, -bet.stake, `Joined bet: ${bet.title} (${option})`, timestamp]);
    ...
    db.run("INSERT INTO Activity (id, circleId, betId, userId, type, data, createdAt) VALUES (?, ?, ?, ?, 'bet_joined', ?, ?)", [cuid(), bet.circleId, betId, auth.id, JSON.stringify({ betTitle: bet.title, option, stake: bet.stake }), timestamp]);
  });
