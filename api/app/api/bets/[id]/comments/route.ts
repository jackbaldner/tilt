...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const id = cuid();
  const timestamp = now();
  await run("INSERT INTO Comment (id, betId, userId, text, createdAt) VALUES (?, ?, ?, ?, ?)", [id, betId, auth.id, text.trim(), timestamp]);
  await run("INSERT INTO Activity (id, circleId, betId, userId, type, data, createdAt) VALUES (?, ?, ?, ?, 'comment', ?, ?)",
    [cuid(), bet.circleId, betId, auth.id, JSON.stringify({ betTitle: bet.title, text: text.trim() }), timestamp]);

  const comment = await one<any>("SELECT * FROM Comment WHERE id = ?", [id]);
  const user = await one<any>("SELECT id, name, image FROM User WHERE id = ?", [auth.id]);
