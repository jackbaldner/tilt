...
export async function GET(req: NextRequest, { params }: { params: Promise<{ code: string }> }) {
  ...
  const circle = one<any>("SELECT * FROM Circle WHERE inviteCode = ?", [code]);
  if (!circle) return NextResponse.json({ error: "Invalid invite code" }, { status: 404 });

  const owner = one<any>("SELECT id, name, image FROM User WHERE id = ?", [circle.ownerId]);
  const memberCount = one<any>("SELECT COUNT(*) as count FROM CircleMember WHERE circleId = ?", [circle.id])?.count ?? 0;
  const betCount = one<any>("SELECT COUNT(*) as count FROM Bet WHERE circleId = ?", [circle.id])?.count ?? 0;
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ code: string }> }) {
  ...
  const existing = one<any>("SELECT * FROM CircleMember WHERE circleId = ? AND userId = ?", [circle.id, auth.id]);
  if (existing) {
  ...
  run(
    ...
    [cuid(), circle.id, auth.id, now()]
  );
  run(
    "INSERT INTO Activity (id, circleId, userId, type, data, createdAt) VALUES (?, ?, ?, 'member_joined', ?, ?)",
    [cuid(), circle.id, auth.id, JSON.stringify({ userName: auth.name }), now()]
  );
