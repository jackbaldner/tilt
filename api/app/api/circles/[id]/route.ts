...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const membership = one<any>("SELECT * FROM CircleMember WHERE circleId = ? AND userId = ?", [id, auth.id]);
  ...
  const betCount = one<any>("SELECT COUNT(*) as count FROM Bet WHERE circleId = ?", [id])?.count ?? 0;
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  return NextResponse.json({ circle: one("SELECT * FROM Circle WHERE id = ?", [id]) });
}
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  run("DELETE FROM Circle WHERE id = ?", [id]);
