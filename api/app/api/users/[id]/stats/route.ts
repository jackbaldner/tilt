...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const user = await one<any>("SELECT id, name, image, chips, createdAt FROM User WHERE id = ?", [userId]);
  if (!user) return NextResponse.json({ error: "User not found" }, { status: 404 });

  const stats = await one<any>("SELECT * FROM UserStats WHERE userId = ?", [userId]);
